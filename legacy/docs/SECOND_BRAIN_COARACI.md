# SECOND_BRAIN.md — reference for Claude (read FIRST after any context compaction)

Audience: me (Claude), future sessions of this project. Purpose: recover context cheaply, avoid re-deriving facts.
Keep it dense. Update section 9 (STATE) at the end of every work block. Long details live in
`~/.claude/projects/-home-users-roarj-work-cp2k/memory/*.md` (index: MEMORY.md) — link, don't copy.

---------------------------------------------------------------------------------------------------
## 0. Rules of engagement (learned the hard way)
...
---------------------------------------------------------------------------------------------------
## 2. Machines
### coaraci (THIS machine; `coaraci.ifi.unicamp.br`; Slurm 23.11.4, Oracle Linux 8)
...
### Target cluster (NOT reachable from here)
56 cores/node, 126 GB/node, partitions `short/medium/long`, `module load cp2k/2026.1`, `CP2K_DATA` set, conda env name unknown, MPI launch flag unknown.
Job default there: `-N 1 --ntasks-per-node=14 --cpus-per-task=2 --mem-per-cpu=2194M`.

---------------------------------------------------------------------------------------------------
## 3. Repo layout (`~/work_cp2k`) — pipeline rewritten from scratch 2026-09-21 per `cp2k_pipeline.md` (the spec)
```
cp2k_pipeline.md         spec (PARTE 1 findings, 2 design, 3 open items, 4 refs)
core/cp2k_blocks.py      ALL input blocks + argparse helpers + structure/restart loaders (single source of truth)
                         BASES registry + MANDATORY `--basis` (dzvp-molopt-sr | ccgrb-d | ccgrb-t | pob-tzvp-rev2 -> files, names, GPW/GAPW, ADMM aux);
                         `--functional PBE|PBE0|HSE06`: k!=Gamma hybrids -> `build_hfx_ri_block` (RI-HFXk, addendum), Gamma hybrids -> `build_xc_block`+MO_DIAG ADMM;
                         `resolve_max_scf` (100 PBE / 300 hybrid); band-path helpers (`band_setup`, shared by 00/04/06); `check_restart_matches_input`
00_basis_probe.py        (addendum) 2 ENERGY inputs, PBE0 RI-HFXk 4x4x4 + band print, reference dzvp-molopt-sr vs `--alt-basis`; run with job_bands_hybrid.sh; parsers/parse_basis_probe.py compares gap+time
01_grid_convergence.py   --phase cutoff|relcutoff; ENERGY, MAX_SCF 1 (+IGNORE_CONVERGENCE_FAILURE), Gamma, OT
02_kmesh_convergence.py  one full CELL_OPT per isotropic NxNxN mesh (kmesh_scan/kNxNxN/)
03_cellopt.py            production single-step CELL_OPT (CELL_REF 1.15x, STRESS_TENSOR ANALYTICAL, BFGS, SCF restart ON -> <proj>-RESTART.kp with k-points)
04_bands.py              PBE only: ENERGY + &PRINT%BAND_STRUCTURE on pymatgen kpath.prim; ADDED_MOS in SCF and BAND_STRUCTURE; EPS_PGF_ORB 1e-12 + restart ON
                         -> writes <proj>-RESTART.kp = SCF guess of stage 6
05_pdos.py               Gamma-only supercell, OT, &PRINT%PDOS NLUMO -1 COMPONENTS
06_bands_hybrid.py       (addendum) same structure/mesh/path as 04, PBE0|HSE06 RI-HFXk, SCF_GUESS RESTART + WFN_RESTART_FILE_NAME <pbe>-RESTART.kp, --kp-ngroups
parsers/parse_{grid,kmesh,cellopt,bands,pdos,basis_probe}.py + common.py   (grid/kmesh/cellopt have --json for the driver; parse_bands.band_edges() shared)
jobs/job_{grid,kmesh,cellopt,bands,bands_hybrid,pdos}.sh    self-contained SLURM jobs (see 7); bands_hybrid aborts unless SLURM_NTASKS % KP_NGROUPS == 0 and the .kp guess exists
scripts/probe_build.sh   probes &DFT%PRINT%DOS(%CURVE) syntax and RI-HFX+k-points on a tiny Si2 cell (run with --mem=0)
scripts/run_smoke.py     unattended 01->05 driver (sbatch --wait per stage, reads parser --json, resubmits incomplete restart-safe jobs)
scripts/{extract_relaxed_structure,parse_cutoff_convergence,parse_kmesh_stress_convergence}.py   old, left in place (extractor has the CELL_REF bug)
legacy/                  old pipeline (input_generator.py etc.) — the HSE06+RI-HFX saga code
smoke/                   rutile.cif (6 atoms), exp/ (HSE06 feasibility inputs+outputs), run1/ (driver run; smoke_state.json, *.json results)
SECOND_BRAIN.md          this file
```
Generators write to the CURRENT dir and import `core` via their own path (run from anywhere). Physics numbers are argparse defaults
(`DEFAULTS` in cp2k_blocks.py); only `.sh` jobs hardcode.

---------------------------------------------------------------------------------------------------
## 4. Spec digest (cp2k_pipeline.md) — decisions already made
1. **PDOS + k-points is invalid in CP2K** (Gamma only) -> PDOS via Gamma-only supercell + OT. 2. ADDED_MOS small (2-10), never derived from n_electrons.
3. k-point runs need `EXTRAPOLATION USE_GUESS`, `&SMEAR` FERMI_DIRAC 300 K, `PARALLEL_GROUP_SIZE -1` (= smallest group per k-point = max k
   parallelism; 0 = all procs), `EPS_ADAPT 0.01`, `CELL_REF`. 4. CELL_OPT already relaxes cell+ions: one step, no GEO_OPT.
5. Grid convergence = official 2D method: ENERGY, MAX_SCF 1, PRINT_LEVEL MEDIUM, Gaussians per grid level; CUTOFF scan (REL_CUTOFF 60), then REL_CUTOFF scan.
6. The old 1.5x REL_CUTOFF for stress was invented; removed. 7. HSE06 bands: to verify (probe). 8. Unified `&DFT%PRINT%DOS`(`&CURVE`) only in CP2K >= 2026.2;
   PDOS broadening in Python. 9. BFGS for <=10 atoms, LBFGS for big cells. Order: 01 -> 02 -> 03 -> {04 bands, 05 pdos}.
Design choices of mine (flag if the user objects): EXTERNAL_PRESSURE written explicitly = 0 bar (CP2K default 100); NGRIDS 4 (manual); ADDED_MOS 10 in BOTH
SCF and BAND_STRUCTURE (=> 20 empty bands, spec-literal); ADDED_MOS/SMEAR only on the k-point branch; no OUTER_SCF.

---------------------------------------------------------------------------------------------------
## 5. Verified CP2K facts (2025.1 unless noted) — reuse, don't re-derive
...
---------------------------------------------------------------------------------------------------
## 6. HSE06 in CP2K — everything learned
...
---------------------------------------------------------------------------------------------------
## 9e. 2026-09-22 (final) — USER PAUSED CP2K, moving to CRYSTAL
...
## 9d. 2026-09-22 (later still) — "traditional DIAGONALIZATION for hybrid k-point bands, no RI" empirically re-confirmed impossible (`smoke/diag_no_ri_test/`)
...
## 9c. 2026-09-22 (later) — untried RI-HFXk manual alternatives, actually tested now (`smoke/ri_flavor_test/`)
...
## 9. STATE (update me) — last updated 2026-09-22 (HFX &MEMORY / PDOS-interface check)
...
## 9b. Prior STATE — 2026-09-21 (after the hybrid-bands addendum)
